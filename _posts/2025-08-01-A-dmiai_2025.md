...
The competition is a week long hackathon where your team is evaluated across 3 different challenges. How many points you get in a task depends on your performance *relative* to the other contestants. The points of the individuals challenges are based on the [Formula 1 point system](https://racingnews365.com/f1-points-system); 1. place gets 25 points, 2. place gets 18 points, 3. place gets 15 points, etc. 

The challenges are formatted in a way, such that we can host an endpoint to expose our model through an API. The API will then have inputs sent to it, for which there are constraints on how long it can take to return a valid answer. 
...
## [](#challenge-1)Challenge 1 - Emergency Healthcare RAG
As if decided by fate, one of the challenges was to design a RAG system, which I just happened to have spent my previous 2 months working on. I had the main responsibility for this task.
...
And then, we fed the chunk text into an LLM with the following prompts, in order to determine whether the statement was true or false, using the reference data:
```python
system_prompt = (
...
This task was actually being recycled from a previous year, because of some trouble with the creators of the task we were supposed to have. We were supposed to work with diseases and eye scan image data. 
...
1. We trained using specific image augmentations that matched the patterns seen in the validation dataset: {salt/pepper noise, horizontal flips, and small perturbations in angle}
2. We created an ensemble of 5 models, where each model was trained on 5 separate folds.
